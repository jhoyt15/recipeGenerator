...
def databaseInitialize(database: recipeDataHandler.RecipeDataHandler) -> None:
    database.executeQuery("CREATE TABLE Recipe (name varchar(255), favorite int, rec_id int NOT NULL PRIMARY KEY)")
    ...
    with open(fileName,encoding = "utf8") as csvFile:
        ...
        for row in rowReader:
            database.executeQuery("INSERT INTO Recipe VALUES ('%s',0,%d)" %(row[0],int(row[1])))
